...
### Kilo Code
Commands live in `.kilo/commands/` (project) or `~/.config/kilo/commands/` (global) and are invoked by filename without the extension — `/kickoff`, `/phase-1`, and so on. The methodology lives in `.kilo/rules/workflow-methodology.md` and is loaded via the `instructions` key in `kilo.jsonc`. Older builds read `.kilocode/workflows/` and `.kilocode/rules/`; both are shipped.

Kilo does not substitute `$ARGUMENTS`, so each generated command opens with an `## Arguments` note telling the agent to read the text typed after the slash command, and to ask via the `question` tool when nothing was typed.

### Other Agents (Antigravity/Codex/Goose)
Workflows (slash commands) live in `.agent/workflows/` (singular — Antigravity only reads this path).
Skills live in `.agents/skills/` (plural — Antigravity/Codex).
Both are auto-generated by `scripts/sync-platforms.sh`. Commands are copied verbatim from `commands/*.md` — frontmatter (`description`, `argument-hint`) is preserved in both `.opencode/commands/` and `.agent/workflows/`. The Kilo Code targets are generated by `scripts/build-kilo.mjs` instead, which rewrites the frontmatter to what Kilo actually reads.
